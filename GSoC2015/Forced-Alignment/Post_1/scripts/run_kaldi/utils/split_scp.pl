...
if ($utt2spk_file ne "") {  # We have the --utt2spk option...
   ...
   # option and we just break into equal size chunks.
    ...
    $numlines = @F;
#tejas: I commented the following if loop.
#    if($numlines == 0) {
#        print STDERR "split_scp.pl: error: empty input scp file $inscp , ";
#        $error = 1;
#    }
    $linesperscp = int( $numlines / $numscps); # the "whole part"..
#    $linesperscp >= 1 || die "You are splitting into too many pieces!";    #tejas: I commented this. 
    $remainder = $numlines - ($linesperscp * $numscps);
#    ($remainder >= 0 && $remainder < $numlines) || die "bad remainder $remainder";   #tejas: I commented this. 
